...
* `cfg` (dict | Settings): A dictionary (or `code_agent.settings.Settings`) containing LLM configuration options (e.g.,
  `ollama_model`, `ollama_host`, `ollama_port`). This is typically loaded from the `.env` file at the project root.
...
![visualizations/code_agent.tools.notebook_tool.NotebookTool.mmd](visualizations/code_agent.tools.notebook_tool.NotebookTool.mmd)

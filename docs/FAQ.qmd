...
**Yes.** The agent is configured via the `.env` file at the project root (see
`.env.example`). You can change `OLLAMA_MODEL` to any other model you have
available in Ollama (e.g., `llama3`, `codellama`, etc.). The `create_llm`
function is designed to be easily adaptable to other local LLM providers as well.
...
1. Create a new Python file in the [`code_agent/tools/`](../code_agent/tools/__init__.py) directory.
2. Define a new class that inherits from `langchain_core.tools.BaseTool`.
3. Implement the `_run` method with your tool's logic.
4. Add your new tool to the `create_default_tools` function in [`code_agent/agents/base_agent.py`](../code_agent/agents/base_agent.py) .

See the existing tools for examples.

## 7. Why does `code-agent chat` sometimes look empty or delayed?

The chat interface now streams live output when possible. If the final model reply is empty, the CLI shows `(empty response)` instead of a blank line. If your backend does not support streaming, the interface falls back to a normal blocking request automatically. Use `--verbose` if you want more visible agent activity during execution.

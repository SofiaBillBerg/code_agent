...
def test_build_agent_returns_runnable(tmp_path: Path, dummy_llm: BaseChatModel) -> None:
...
def test_build_agent_with_invalid_config(tmp_path: Path) -> None:
    ...
    assert isinstance(agent_runnable, Runnable), ("build_agent should return a Runnable even with fallback LLM")

    # Test invocation to ensure it returns the error message
    response = agent_runnable.invoke({"messages": [HumanMessage(content = "test")]})

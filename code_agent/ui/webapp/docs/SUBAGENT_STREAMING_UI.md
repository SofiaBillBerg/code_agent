...
- `code_agent/ui/protocol.py` always emits `message-start` even for `content=""` → `useCustomStream` was pushing empty
  `{text:""}`.
- Sub-agent namespaces (`ns.length>0`) were also pushed to root stream.

**Fix applied (commit pending):**

- `code_agent/ui/webapp/src/useCustomStream.js:72-124` — ignore non-root namespaces, drop empty `message-finish`
  (`isEmpty → filter`), lazy-create still preserved.
- `code_agent/ui/webapp/src/App.jsx:110-116` — `messages.filter(hasText||hasTools)` so empty markdown not rendered.
...
- File: `code_agent/ui/webapp/src/components/SubagentTree.jsx`
